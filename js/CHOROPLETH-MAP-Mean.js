// initial setup
function CHOROPLETHMAPMean() {
	const svg = d3.select("#CHOROPLETHMAP-MEAN"),
		width = svg.attr("width"),
		height = svg.attr("height"),
		path = d3.geoPath(),
		data = d3.map(),
		worldmap = "https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson",
		worldpopulation = "./Map/All_countries_MapChart.csv";
	...
	function ready(error, topo) {
		...
		let mouseOver = function(d) {
			...
			tooltip.style("left", (d3.event.pageX + 15) + "px")
				...
				.style("opacity", 1)
				.text(d.properties.name + ': ' + d.total + ' years');
		}
		...
		.text(function(d, i) {
				if (i === 0) return "< " + d[1] + " years";
				if (d[1] < d[0]) return d[0] + " years +";
				return d[0] + " years - " + d[1] + " years";
			});

		legend.append("text").attr("x", 15).attr("y", 580).text("Life expectancy (years)");
	}

function LastQuery_INFD(){
  ...
  var svg = d3.select("#LastQ_INFD_LineChart")
    .append("svg")
  ...
  //Read the data
  d3.csv("./LineCharts/LastQuery_LineChart.csv", function(data) {

      // List of groups (here I have one group per column)
      var allGroup = d3.map(data, function(d){return(d.Country)}).keys()

      // add the options to the button
      d3.select("#selectButton_TQ_INFD_LineChart")
        .selectAll('myOptions')
        .data(allGroup)
        .enter()
        .append('option')
  ...
  */
      svg.append("g")
        ...
        // Add Y right axis
      var yy = d3.scaleSqrt()
        .domain([0, d3.max(data, function(d) { return +d.infantDeaths; })])
        .range([ height, 0 ]);
  ...
  */
      svg.append("g")
        ...
        svg.append("text")
                    ...
                    .attr("y", 40)
                    .text("Infant Deaths (per 1000 population)");
      ...
      var line2 = svg
        ...
        .append("path")
          ...
          .attr("d", d3.line()
            .x(function(d) { return x(d.Year) })
            .y(function(d) { return yy(+d.infantDeaths) })
          )
      ...
      d3.select("#selectButton_TQ_INFD_LineChart").on("change", function(d) {

function SecondQuery(){
  ...
  var svg = d3.select("#SecondQ_LineChart")
    .append("svg")
  ...
  //Read the data
  d3.csv("LineCharts/SecondQuery_LineChart.csv", function(data) {

      // List of groups (here I have one group per column)
      var allGroup = d3.map(data, function(d){return(d.Country)}).keys()

      // add the options to the button
      d3.select("#selectButton_SQ_LineChart")
        .selectAll('myOptions')
        .data(allGroup)
        .enter()
        .append('option')
  ...
  */
      ...
      var yy = d3.scaleLinear()
        .domain([0, d3.max(data, function(d) { return +d.Schooling; })])
        .range([ height, 0 ]);
  ...
  */
      ...
      var line2 = svg
        ...
        .append("path")
          ...
          .attr("d", d3.line()
            .x(function(d) { return x(d.Year) })
            .y(function(d) { return yy(+d.Schooling) })
          )
      ...
      d3.select("#selectButton_SQ_LineChart").on("change", function(d) {

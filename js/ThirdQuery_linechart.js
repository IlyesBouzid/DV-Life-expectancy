function ThirdQuery_HepB(){
  ...
  var svg = d3.select("#ThirdQ_HepB_LineChart")
    .append("svg")
  ...
  //Read the data
  d3.csv("./LineCharts/ThirdQuery_LineChart.csv", function(data) {

      // List of groups (here I have one group per column)
      var allGroup = d3.map(data, function(d){return(d.Country)}).keys()

      // add the options to the button
      d3.select("#selectButton_TQ_HepB_LineChart")
        .selectAll('myOptions')
        .data(allGroup)
        .enter()
        .append('option')
  ...
  */
      ...
      var yy = d3.scaleLinear()
        .domain([0, d3.max(data, function(d) { return +d.HepatitisB; })])
        .range([ height, 0 ]);
  ...
  */
      svg.append("g")
        ...
        svg.append("text")
                    ...
                    .attr("y", 40)
                    .text("Hepatitis B (Immunization factor)");
      ...
      var line2 = svg
        ...
        .append("path")
          ...
          .attr("d", d3.line()
            .x(function(d) { return x(d.Year) })
            .y(function(d) { return yy(+d.HepatitisB) })
          )
      ...
      d3.select("#selectButton_TQ_HepB_LineChart").on("change", function(d) {
